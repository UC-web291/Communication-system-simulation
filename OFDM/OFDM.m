...
% 生成一个长度为 carrier_count * symbol_count * bit_per_symbol 的随机 0-1 序列，并转置为列向量。
% ================子载波调制方式1========================
%因为不能所有的载波都携带复数数据，做ifft的点数N是相当于对时域发送信号的采样，
% 要满足采样定理，所以N至少是实际数据子载波个数的两倍。然后是带通信道的话，
% 前面必须要置零一部分，因为有些载波的中心频率是不在那个带通信道带宽内的，
% 但是这个前置的0和后续的频域补零的零它不是一个含义，一个是你认为它就是携带了0，
% 另外一个是为了满足点数N要求去频域补零，这样才能出来512个时域采样点
% 1-28置零 29-228有效 229-285置零 286-485共轭 486-512置零
%%这段程序构造了512个子载波的OFDM符号，并且各个子载波上的数据是共轭对称的。
% 这样做的目的是经过%IFFT后形成的OFDM符号均为实数。另外，在512个子载波中，
% 仅有400个子载波为数据，其余为0值，相当于补零，补零的目的是通常IFFT的长度应该为2的整数次幂。
carrier_position = 29:228;
conj_position = 485:-1:286;
bit_moded = qammod(bit_sequence,16,'InputType','bit');
% 定义子载波的有效位置和共轭位置，使用 QAM16 调制随机序列，输入类型为 bit。
...
% 创建一个全零矩阵 ifft_position，将调制后的序列重新整形为子载波数和符号数的矩阵。
...
% 将有效子载波位置和共轭子载波位置赋值为调制后的序列，对 ifft_position 进行 IFFT 变换得到时域信号 signal_time。
...
% 在时域信号 signal_time 上添加循环前缀（从末尾取 CP_length 个样本放在开头）和循环后缀（取开头的 CS_length 个样本放在末尾）。
signal_time_C = [signal_time_C; signal_time(1:CS_length,:)];
...
% 通过矩阵点乘
% 创建与 signal_time_C 相同大小的全零矩阵，然后使用升余弦窗函数对信号进行加窗操作。
...
% 将加窗后的信号 reshape 为一维向量作为待传输信号 signal_Tx，同时保存未加窗的完整信号 signal_origin。
% 定义多径幅度和多径时延，模拟多径信道，生成多径信号 signal_Tx_mult。
figure('menubar','none')
subplot(2,1,1)
plot(signal_Tx_mult)
title('多径下OFDM信号')
